...
function checkWordGuess() {
    ...
    // increment or decrement the score based on the guess
    score = score + 1*(userGuess == wordToGuess) - 1*(userGuess != wordToGuess);
    scoreDisplay.innerHTML = `Score: ${score}`;
    
    // create a counter element to provide feedback on the score change
    ...
    // add the counter element and remove it right after the animation effect finishes
    scoreDisplay.appendChild(counterElement);
    setTimeout(() => scoreDisplay.removeChild(counterElement), 1000);

    // if the guess was correct, generate a new word and provide feedback on new word generation
...
// rotate the hand while dragging the mouse
...
// quit dragging and reset cursor on releasing the mouse
...
// set up for hand rotation if touch is starting on the canvas only, not on the UI area
...
// quit dragging on touch end or cancellation
...
// rotate the hand while dragging by touch
...
//================================================================================
    // Handle shortcuts and UI content interaction
//================================================================================

// update the height of the collapsible content if it is currently open
...
// toggle the collapsible content to open or close - height of the collapsible will not be changed if it is closed
...
// update the right-side value displays of range sliders to reflect the current slider value
...
async function init() {
    ...
    // update range slider styles and values
    updateRangeSliderStyle(speedSlider);
    updateRangeSliderStyle(angleSlider);
